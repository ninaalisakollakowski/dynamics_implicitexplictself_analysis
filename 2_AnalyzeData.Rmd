...
imputedData <- mice(currentData_scaled, 
                    m = nImputations,
                    print = FALSE, 
                    seed = 350,
                    where = )
...
SRMR < .11 (0.101) -> good
CFI < .95 (0.440) -> bad
3/4 indicators show bad fit of CFA for implicit measures -> perform EFA
...
```
significant X2-test (p = 0.021) -> bad
RMSEA > .08 (0.087) -> bad
SRMR < .11 (0.072) -> good
CFI < .95 (0.684) -> bad
...
#exclude variables that don't load on any factor with loading of >=.4
excludedVariables_explicit <- c("selfUnderstanding_passed",
                                "videoSelfRecognition",
                                "bodyObstacle_errors")
```
...
#for dichotomous excluded variables, make sure they are either 1 (=0) or 2 (=1) (as averaging of the imputed datasets might produce fractionals)
explicitSelf$videoSelfRecognition <- ifelse(explicitSelf$videoSelfRecognition > 1.5,2,1)
```
...
videoSelfRecognition.model <- lm(videoSelfRecognition ~ implicitFactor1 + 
                              implicitFactor2 + 
                              implicitFactor3 + 
                              temporalContingency_left + 
                              temporalContingency_looking + 
...
```
```{r body obstacle}
bodyObstacle.model <- lm(bodyObstacle_errors ~ implicitFactor1 + 
                              implicitFactor2 + 
                              implicitFactor3 + 
                              temporalContingency_left + 
                              temporalContingency_looking + 
