...
```
significant X2-test (p = 0.005) -> bad
RMSEA > .08 (0.102) -> bad
SRMR < .11 (0.078) -> good
CFI < .95 (0.647) -> bad
...
#run KMO again
KMO(correlation_implicit1) #KMO = 0.51 -> exclude reaching_head (0.44)
...
as EFA exceeds maximum iteration, trying to exclude all variables with KMO <.5 iteratively
```{r KMO}
KMO(correlation_explicit) #KMO = 0.52, however EFA produces Heywood case so excluding variables until all variables have individual KMO >=0.5 -> excluding bodyObstacle_errorFree (0.29)
...
KMO(correlation_explicit1) #KMO = 0.54, but variables <.5 -> excluding bodySizeToys_errorFree (0.35
