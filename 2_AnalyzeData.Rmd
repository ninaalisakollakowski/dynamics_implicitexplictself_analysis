...
```{r KMO}
KMO(correlation_implicit) #KMO = 0.46, exclude reaching_head (0.42)
...
#run KMO again
KMO(correlation_implicit1) #KMO = 0.48, exclude reaching_arm (0.40)
correlation_implicit2 <- correlation_implicit1[!rownames(correlation_implicit1) == "reaching_arm",
                                              !colnames(correlation_implicit1) == "reaching_arm"]
...
#run KMO again
KMO(correlation_implicit2) #KMO = 0.49, exclude temporalContingency_right (0.43)
correlation_implicit3 <- correlation_implicit2[!rownames(correlation_implicit2) == "temporalContingency_right",
                                              !colnames(correlation_implicit2) == "temporalContingency_right"]
excludedVariables_implicit_KMO <- append(excludedVariables_implicit_KMO, "temporalContingency_right")
excludedVariables_implicit <- append(excludedVariables_implicit, "temporalContingency_right")

#run KMO again
KMO(correlation_implicit3) #KMO = 0.5 -> proceed
...
efa.implicit <- fa(cur_correlation_implicit, nfactors = 3, n.obs = nObservations, 
...
excludedVariables_implicit <- append(excludedVariables_implicit, "sensoryAttenuation_pos")
...
2) KMO >.5, otherwise exclude variables with smallest KMO until criterion is reached
```{r KMO}
KMO(correlation_explicit) #KMO = 0.52, however EFA exceeds maximum iterations so excluding variables until all variables have individual KMO >=0.5 -> excluding bodyObstacle_errorFree (0.34)
