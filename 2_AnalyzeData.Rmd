...
demographicVariables_numeric <- c("education_mother",
                                  ...
                                  "age_T3")
demographicVariables_factors <- c("Marital_Status_num",
                                  "Siblings_num",
...
demo_marital_status <- table(demographics_num$Marital_Status_num, demographics_num$time)
chisq.test(demo_marital_status)
```

```{r siblings}
# (0 = no siblings, 1 = at least one sibling) -> not significant
demo_siblings <- table(demographics_num$Siblings_num, demographics_num$time)
chisq.test(demo_siblings)
```

```{r language}
#primary language of child (1 = german, 0 = other) -> significant
demo_language <- table(demographics_num$Primary_Language_Child_num, demographics_num$time)
chisq.test(demo_language)
#more German speaking children contributed data only to T1 (9/0) than to T1 and T2 (5/2) than contributed data to all time points (96/4)
```

```{r caretaker}
#primary caretaker at T1 (0 = mother, 1 = father, 2 = both) -> not significant
demo_caretaker <- table(demographics_num$Primary_Caretaker_num, demographics_num$time)
chisq.test(demo_caretaker)
```

```{r sex}
#sex of child (m = male, w = female) -> significant
demo_sex <- table(demographics_num$sex, demographics_num$time)
chisq.test(demo_sex)
...
#check if data is not MCAR because of dropouts
naniar::mcar_test(currentData %>% 
                    filter(if_any(all_of(explicitSelfMeasures), ~ !is.na(.)))) # p = .073 -> not the case

#check if data is not MCAR because of neuro-cognitive measures
naniar::mcar_test(currentData %>% 
                    select(-starts_with("temporalContingency"),
                           -starts_with("sensoryAttenuation"))) # p = .323 -> this seems to be the case
```
...
6) calculate factor scores
```{r factor scores}
#initialize lists for factor scores
factorScores_explicit <- vector("list", nImputations)

#calculate factor scores for each imputed dataset
for (i in 1:nImputations){
  curDataset <- complete(imputedData, action = i)
...
#create average factor scores for all datasets
factorScores_explicit_avg <- apply(simplify2array(factorScores_explicit), c(1,2), mean)

#combine factor scores into dataframe for explicit self
